...
**_DHCP_** _(англ. Dynamic Host Configuration Protocol — протокол динамической настройки узла)_ — прикладной протокол, позволяющий сетевым устройствам автоматически получать IP-адрес и другие параметры, необходимые для работы в сети TCP/IP. Данный протокол работает по модели «клиент-сервер».
...
###### Для задания статичных настроек редактируем файл `/etc/netplan/00-installer-config.yaml` и `netplan apply` применяем изменения
...
###### Повторный вызов команды `apt upgrade`
...
***Recv-Q, Send-Q***  
Cколько данных находится в очереди для этого сокета, ожидая чтения (Recv-Q) или отправки (Send-Q). Если это 0, то данных в очереди нет.  
***Local/Foreign Address***  
Указывают, к каким хостам и портам подключены перечисленные сокетs. Если в качестве адреса отображается 0.0.0.0, то это означает - "любой адрес", т.е в соединении могут использоваться все IP-адреса существующие на данном компьютере.  
***State***  
...
![Вывод команды fdisk -l ](screenshots/fdisk.png)
- *название жесткого диска* - sda
-  *размер* - 25 GiB
- *количество секторов* - 52428800
- *размер swap* - 1007.5 kB (для того, чтобы иметь доступ к swap через fdisk, создаем новый partition c типом Linux Swap и активируем его)

## Part 11. Использование утилиты **df**
**== Задание ==**

#### Запустить команду df.
![Вывод команды df ](screenshots/df.png)

***Для корневого раздела (/):***  
    -   *размер раздела* - 11758760  
    -   *размер занятого пространства* - 4939668  
    -   *размер свободного пространства* - 6199984  
    -   *процент использования* - 45  
*Единица измерения в выводе:* kB

#### Запустить команду df -Th.
![Вывод команды df  - Th](screenshots/df_th.png)
...
*Единица измерения в выводе:* GB

## Part 12. Использование утилиты **du**
**== Задание ==**

#### Запустить команду du.
![Вывод команды du](screenshots/du.png)

#### Вывести размер папок /home, /var, /var/log (в байтах, в человекочитаемом виде)
##### `du -h /home`
![Размер папки /home](screenshots/du_home.png)

##### `du -h /var` 
![Размер папки /var](screenshots/du_var.png)

##### `du -h /var/log`
![Размер папки /var/log](screenshots/du_varlog.png)

#### Вывести размер всего содержимого в /var/log (не общее, а каждого вложенного элемента, используя *)
##### `du -Th /var/log/*`
![Размер файлов в папке /var/log](screenshots/du_all_var.png)
...
##### `ncdu /home`
![Размер папки /home](screenshots/ncdu_home.png)

##### `ncdu /var` 
![Размер папки /var](screenshots/ncdu_var.png)

##### `ncdu /var/log`
![Размер папки /var/log](screenshots/ncdu_varlog.png)

## Part 14. Работа с системными журналами
**== Задание ==**
##### Открыть для просмотра  /var/log/dmesg, /var/log/syslog, /var/log/auth.log
##### Содержимое `/var/log/auth.log`
![Содержимое /var/log/auth.log](screenshots/auth_log.png)
-   *время последней успешной авторизации* - 13:41:55
-  *имя пользователя* - root
-  *метод входа в систему* - sudo:session
##### Сообщение о рестарте службы SSHd
![Содержимое файла /var/log/syslog](screenshots/sshd_restarted.png)
...
- Вывести список текущих заданий для CRON 
##### Можно узнать при помощи `crontab -l`
![Список текущих заданий cron](screenshots/current_tasks.png)

- Узнать информацию о выполнении
##### Логи из файла `/var/log/syslog`
![Информация о выполнении uptime](screenshots/cron_logs.png)

- Удалить все задания из планировщика заданий.
##### Удалить изменения можно, изменив файл crontab при помощи `crontab -e`
![Удаленные задания из cron](screenshots/deleted_tasks.png)
